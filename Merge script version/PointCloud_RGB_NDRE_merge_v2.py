...
def findPoints(point1,point2):
    ...
    with open("P:/PointCloud_RGB_NDRE_merge/test.xyz", 'a') as file:
        while(currentGroup != maxGroup):
            ...
            while(currentPoint != maxPoint):
                ...
                for i in range(comparisonNum):
                    ...
                    if(currentDistanceValue < closestDistanceValue):
                        ...
                        closestIndex = i
                    if(currentDistanceValue <= 0.01):
                        closestIndex = i
                        print("break distance 0")
                        break
            ...
            currentPoint = 0
            #break the loop is the second file get to the end of the points
            if(secondGroupIndex == len(secondList.getList())):
                print('break the second list is done!')
                break
    return None
...
NDREpoints = ListOfPoint('P:/PointCloud_RGB_NDRE_merge/testA_NDRE.xyz')
RGBpoints = ListOfPoint('P:/PointCloud_RGB_NDRE_merge/testA_RGB.xyz')

findPoints(NDREpoints,RGBpoints)
