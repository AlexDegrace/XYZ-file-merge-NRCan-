...
def findPoints(point1,point2,fileLocation,fileName):
    ...
    currentGroup = 0
    currentPoint = 0
    maxGroup = math.floor(len(firstList.getList())/4)
...
def findPoints2(point1,point2,fileLocation,fileName):
    #Check witch file as the bigger minimum and set this one as the main file
    if(point1.getList()[0][0].getX() >= point2.getList()[0][0].getX()):
        firstList = point1
        secondList = point2
        ...
        secondList = point1
        master = "RGB"
    print("The main image is the " + master + "2")

    distanceLimit = 0.001
    #These index is use to follow the main list and make sure it
    #dosent get out of bound
    currentGroup = math.floor(len(firstList.getList())/4)
    currentPoint = 0
    maxGroup = math.floor(len(firstList.getList())/2)
    secondGroupIndex = secondList.getIndex(firstList.getList()[currentGroup][currentPoint].getX())

    filePath = fileLocation + "/" + "2" + fileName 
    with open(filePath, 'a') as file:
        while(currentGroup != maxGroup):
            #The biggest index of a certain group of the first list
            maxPoint = len(firstList.getList()[currentGroup])
            while(currentPoint != maxPoint):
                ...
                for i in range(comparisonNum):
                    ...
                    if(currentDistanceValue <= distanceLimit):
                        closestIndex = i
                        print("break distance " + str(distanceLimit) + "m2")
                        break
                secondPoint = secondList.getList()[secondGroupIndex][closestIndex]
                #write to the XYZ file
            ...
            #break the loop if the second file get to the end of the points
            if(secondGroupIndex == len(secondList.getList())):
                print('Stop the loop the second list is done2!')
                break
    return None

def findPoints3(point1,point2,fileLocation,fileName):
    #Check witch file as the bigger minimum and set this one as the main file
    if(point1.getList()[0][0].getX() >= point2.getList()[0][0].getX()):
        firstList = point1
        secondList = point2
        ...
        secondList = point1
        master = "RGB"
    print("The main image is the " + master + "3")

    distanceLimit = 0.001
    #These index is use to follow the main list and make sure it
    #dosent get out of bound
    currentGroup = math.floor(len(firstList.getList())/2)
    currentPoint = 0
    maxGroup = math.floor(3*len(firstList.getList())/4)
    secondGroupIndex = secondList.getIndex(firstList.getList()[currentGroup][currentPoint].getX())

    filePath = fileLocation + "/" + "3" + fileName 
    with open(filePath, 'a') as file:
        while(currentGroup != maxGroup):
            #The biggest index of a certain group of the first list
            maxPoint = len(firstList.getList()[currentGroup])
            while(currentPoint != maxPoint):
                ...
                for i in range(comparisonNum):
                    ...
                    if(currentDistanceValue <= distanceLimit):
                        closestIndex = i
                        print("break distance " + str(distanceLimit) + "m3")
                        break
                secondPoint = secondList.getList()[secondGroupIndex][closestIndex]
                #write to the XYZ file
            ...
            #break the loop if the second file get to the end of the points
            if(secondGroupIndex == len(secondList.getList())):
                print('Stop the loop the second list is done3!')
                break
    return None

def findPoints4(point1,point2,fileLocation,fileName):
    #Check witch file as the bigger minimum and set this one as the main file
    if(point1.getList()[0][0].getX() >= point2.getList()[0][0].getX()):
        firstList = point1
        secondList = point2
        ...
        secondList = point1
        master = "RGB"
    print("The main image is the " + master + "4")

    distanceLimit = 0.001
    #These index is use to follow the main list and make sure it
    #dosent get out of bound
    currentGroup = math.floor(3*len(firstList.getList())/4)
    currentPoint = 0
    maxGroup = len(firstList.getList())
    secondGroupIndex = secondList.getIndex(firstList.getList()[currentGroup][currentPoint].getX())

    filePath = fileLocation + "/" + "4" + fileName 
    with open(filePath, 'a') as file:
        while(currentGroup != maxGroup):
            #The biggest index of a certain group of the first list
            maxPoint = len(firstList.getList()[currentGroup])
            while(currentPoint != maxPoint):
                ...
                for i in range(comparisonNum):
                    ...
                    if(currentDistanceValue <= distanceLimit):
                        closestIndex = i
                        print("break distance " + str(distanceLimit) + "m4")
                        break
                secondPoint = secondList.getList()[secondGroupIndex][closestIndex]
                #write to the XYZ file
            ...
            #break the loop if the second file get to the end of the points
            if(secondGroupIndex == len(secondList.getList())):
                print('Stop the loop the second list is done4!')
                break
    return None

def startProcessing(NDREpoints,RGBpoints,filePathMerge,fileName):
    #This function split the processing load in 4 and excute
    #each part in parallel.
    p1 = Process(target = findPoints, args =(NDREpoints, RGBpoints, filePathMerge, fileName,))
    p2 = Process(target = findPoints2, args =(NDREpoints, RGBpoints, filePathMerge, fileName,))
    p3 = Process(target = findPoints3, args =(NDREpoints, RGBpoints, filePathMerge, fileName,))
    p4 = Process(target = findPoints4, args =(NDREpoints, RGBpoints, filePathMerge, fileName,))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
...
def mergeAll(fileLocation, fileName):
    #This function append the file 2,3 and 4 to the first file.
    filePathFirst = fileLocation + "/" + fileName
    filePathSecond = fileLocation + "/" + "2" + fileName
    filePathThird = fileLocation + "/" + "3" + fileName
    filePathFourth = fileLocation + "/" + "4" + fileName
    with open(filePathFirst, 'a') as firstFile:
        
        with open(filePathSecond, 'r') as secondFile:
            secondFileContent = secondFile.readlines()
            for i in secondFileContent:
                firstFile.writelines(i)
                
        with open(filePathThird, 'r') as thirdFile:
            thirdFileContent = thirdFile.readlines()
            for i in thirdFileContent:
                firstFile.writelines(i)

        with open(filePathFourth, 'r') as fourthFile:
            fourthFileContent = fourthFile.readlines()
            for i in fourthFileContent:
                firstFile.writelines(i)
