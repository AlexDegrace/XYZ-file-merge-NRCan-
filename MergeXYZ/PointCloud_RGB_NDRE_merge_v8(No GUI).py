...
def startProcessing(NDREpoints,RGBpoints,filePathMerge,fileName, numProc):
    ...
    for i in range(numProc):
        processes.append(Process(target = findPoints, args =(NDREpoints, RGBpoints, filePathMerge, fileName,numProc, i)))
    for j in processes:
        j.start()
    for q in processes:
        q.join()
...
def startProgramme():
    ...
    fileName =  input("Enter the result file name (add .xyz at the end): ")
    #Creating the list of points from the two .xyz file
    NDREpoints = ListOfPoint("P:/PointCloud_RGB_NDRE_merge/testB_NDRE.xyz") #input("Enter the full path of the NDRE .xyz file (Make sure to put the .xyz at the end): ")
    print("The list base on the NDRE file was created!")
    RGBpoints = ListOfPoint("P:/PointCloud_RGB_NDRE_merge/testB_RGB.xyz") #input("Enter the full path of the RGB .xyz file (Make sure to put the .xyz at the end): ")
    print("The list base on the RGB file was created!")
    # The path where the result gonna be save
    filePathMerge = "P:/PointCloud_RGB_NDRE_merge/TestB_Split8" #input("Enter the full path where you want to put the result: ")
    #Number of process the main task is gonna be split in
    numOfProc = int(input("Number of process: "))
    myTime = time.time()
    #Call the function startProcessing with all the arguments chosen above
    startProcessing(NDREpoints, RGBpoints, filePathMerge, fileName,numOfProc)
    #Merge all of the different process in one file
    mergeAll(filePathMerge,fileName,numOfProc)
