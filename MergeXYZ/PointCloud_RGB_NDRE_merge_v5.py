...
#   in parallel so the computing time is almost cut in two. To do so I have use the multiprocessing
...
def findPoints(point1,point2,fileLocation,fileName):
    ...
    secondGroupIndex = secondList.getIndex(firstList.getList()[0][0].getX())
...
def findPoints2(point1,point2,fileLocation,fileName):
    ...
    #These index is use to follow the main list and make sure it
    #dosent get out of bound
    currentGroup = math.ceil(len(firstList.getList())/2)
    currentPoint = 0
    maxGroup = len(firstList.getList())
    secondGroupIndex = secondList.getIndex(firstList.getList()[currentGroup][0].getX())
...
def mergeAll(fileLocation, fileName):
    ...
    filePathLast = fileLocation + "/" + "2" + fileName
    with open(filePathFirst, 'a') as firstFile:
        with open(filePathLast, 'r') as lastFile:
            lastFileContent = lastFile.readlines()
            for i in lastFileContent:
